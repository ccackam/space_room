...
building = task("Building/Instilation",{project,0});
    ...
    circuit_scramble = task("Circuit Scramble",{building,0},1);
        ...
        circuit_scramble_frame = task("Frame",{circuit_scramble_prototype,3},4);
            circuit_scramble_frame.materials = [two_by_two,pretty_plywood,decorative_steel];
    ...
    switch_board = task("Switch Board",{building,0});
        ...
        switch_board_electrical = task("Electrical",{switch_board_frame,1},3);
            switch_board_electrical.materials = [arduino_uno,wires,mosfet,usb_b,usb_adaptor,switch_2p,light_wire*4,aux_plug*2,aux_cable*3];
    ...
    lasers = task("Lasers",{floor,1});
        ...
        lasers_frame = task("Frame",{lasers_prototype,1},3);
            lasers_frame.materials = two_by_two;
        lasers_electrical = task("Electrical",{lasers_frame,1},2);
            lasers_electrical.materials = [arduino_uno,wires,mosfet,usb_b,usb_adaptor,laser*4];
        lasers_install = task("Install",{lasers_electrical,1},1);
    escape_pod = task("Emergency Exit Door",{teleport,1},5);
    ...
    constilation = task("Constilation Viewer",{paint,1});
        ...
        constilation.add_subtask([constilation_ladder,constilation_lights,constilation_bubble]);
    ...
    ipads = task("iPads",{building,0});
        ipads.materials = 3*iPad;
        code_ipads = task("Code",{ipads,0},20);
        install_ipads = task("Install",{desk,1},2);
            install_ipads.starts_when(electrical,1);
            install_ipads.materials = iPad_mount*3;
        ipads.add_subtask([code_ipads,install_ipads]);
    seats = task("Seats",{floor,1},2);
        seats.materials = [station_seats*2,station_stand*2,station_swivel*2];
    ...
    alien = task("FerroFluid Alien",{building,0},1);
        alien_waterproof = task("Waterproof",{building,0},4);
            alien_waterproof.materials = [waterproofing_plastic,waterproof_caulk];
        alien_install = task("Install",{alien_waterproof,1},1);
            alien_install.materials = [ferrotic_fluid,alien_table,thin_plexi,aluminum_angle*4,two_by_two];
            alien_install.starts_when(electrical,1)
        alien.add_subtask([alien_waterproof,alien_install]);
    moon = task("Moon",{electrical,1},1);
    	moon.materials = [levitating_moon,thin_plexi,aluminum_angle*4,two_by_two];
        moon.starts_when(desk,1);
        moon.starts_when(electrical,1);
    hatch = task("Hatch Entrance",{demolition,1});
        hatch_cut_hole = task("Cut Hole",{hatch,0},2);
            floor.starts_when(hatch_cut_hole,1);
        hatch_frame = task("Frame",{hatch_cut_hole,1},5);
            hatch_frame.materials = [two_by_four,pretty_plywood,decorative_steel];
        hatch_paint = task("Paint",{hatch_frame,1},3);
        hatch_door = task("Door",{hatch_frame,1},2);
        hatch_lock = task("Lock",{hatch_door,1},4);
            hatch_lock.materials = magnet_lock;
            hatch_lock.starts_when(electrical,1);
        hatch_ladder = task("Ladder",{hatch_cut_hole,1},1);
            hatch_ladder.materials = [ladder,ladder_screws];
        hatch.add_subtask([hatch_cut_hole,hatch_frame,hatch_paint,hatch_door,hatch_lock,hatch_ladder]);
    ...
    crackel = task("Crackle Tube",{building,0});
        crackel_powersupply = task("Power Suply",{crackel,0},1);
            crackel_powersupply.materials = power_supply*3;
        ...
        crackel_frame = task("Frame",{drive_platform,1},4);
            crackel_frame.materials = [pretty_plywood,two_by_four,two_by_ten,two_by_two*4,thick_plexi,plasma_tube,crackel_tube,lightning_tube];
    ...
    infinity_mirrors = task("Infinity Mirrors",{glass_walls,1});
        infinity_mirrors_frame = task("Frame",{infinity_mirrors,0},4);
            infinity_mirrors_frame.materials = [infinity_frame*2,infinity_mirror*2,infinity_glass_film*2,infinity_plexiglass*2,infinity_wire_support,grate];
    ...
    starchart = task("Start Chart TV",{building,0});
        starchart_animation = task("Animation",{starchart,0},7);
        starchart_frame = task("Frame",{paint,1},4);
            starchart_frame.materials = two_by_two*3;
        starchart_electrical = task("Electrical",{starchart_frame,1},2);
        starchart_paint = task("Paint",{starchart_electrical,1},3);
        starchart_TV = task("Mount Television",{starchart_paint,1},3);
            starchart_TV.materials = [tv_55,tv_mount,thick_plexi];
        starchart.add_subtask([starchart_animation,starchart_frame,starchart_electrical,starchart_paint,starchart_TV]);
