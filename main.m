...
point_LED = item("EDGELEC 100pcs 5mm RGB Tri-Color",8.99,"https://www.amazon.com/EDGELEC-Tri-Color-Multicolor-Diffused-Resistors/dp/B077XGF3YR/ref=sr_1_3?keywords=rgb+led&qid=1557420126&s=gateway&sr=8-3");
...
thin_plywood = item("5.2mm - Sande Plywood",22.92,"https://www.homedepot.com/p/Sandeply-5-2mm-Sande-Plywood-1-4-in-Category-Common-1-4-in-x-4-ft-x-8-ft-Actual-0-205-in-x-48-in-x-96-in-479023/203414042");
...
rasberry_pie = item("CanaKit Raspberry Pi 3 B+ (B Plus) with Premium Clear Case and 2.5A Power Supply",54.99,"https://www.amazon.com/CanaKit-Raspberry-Premium-Clear-Supply/dp/B07BC7BMHY/ref=sr_1_1?keywords=raspberry+pi&qid=1557791643&s=gateway&sr=8-1");
...
variable_voltage_adaptor = item("Belker Universal 3V 4.5V 5V 6V 7.5V 9V 12V AC DC Adapter",11.9,"https://www.amazon.com/Belker-Universal-Household-Electronics-Speakers/dp/B072L81WX8/ref=sr_1_2?crid=24YY2OJUBTZTJ&keywords=variable+voltage+adapter&qid=1557790909&s=gateway&sprefix=variable+voltage+adap%2Caps%2C177&sr=8-2");
...
tool_box = item("20 in. Tread Plate Metal Mobile Tool Box",27.97,"https://www.homedepot.com/p/Husky-20-in-Tread-Plate-Metal-Mobile-Tool-Box-TB-520/202279042");
...
toilet_plug = item("3 in. Gripper Plastic Mechanical Test Plug",5.57,"https://www.homedepot.com/p/Oatey-3-in-Gripper-Plastic-Mechanical-Test-Plug-33402/100168770");
liquid_nails = item("10 oz. Extreme Heavy Duty Adhesive",3.47,"https://www.homedepot.com/p/Liquid-Nails-10-oz-Extreme-Heavy-Duty-Adhesive-LN-907/205089262");

infinity_mirror = item("Havana Pieced Mirror Black 24x24 - Opalhouse", 49.99, "https://www.target.com/p/havana-pieced-mirror-black-24x24-opalhouse-153/-/A-53019971");
...
construction = task("Contractor Construction",{project,0});
    ...
    desk = task("Desk",{construction,0});
        ...
        desk_electrical = task("Electrical",{base,1},1);
            desk_electrical.materials = [wires,2*extension_cord];
        top = task("Top",{desk_electrical,1},5);
            top.materials = 16*thin_plywood;
...
building = task("Building/Instilation",{project,0});
    ...
    circuit_scramble = task("Circuit Scramble",{building,0},1);
        ...
        circuit_scramble_frame = task("Frame",{circuit_scramble_prototype,3},4);
            circuit_scramble_frame.materials = [two_by_two,pretty_plywood,decorative_steel,pretty_plywood,thin_plywood];
        circuit_scramble_electrical = task("Electrical",{circuit_scramble_frame,1});
            circuit_scramble_electrical.materials = [arduino_mega,arduino_uno,wires*2,mosfet*7,usb_b*2,usb_adaptor*2,switch_2p*5,light_wire*2,tool_box];
    ...
    switch_board = task("Switch Board",{building,0});
        ...
        switch_board_electrical = task("Electrical",{switch_board_frame,1},3);
            switch_board_electrical.materials = [arduino_uno,wires,mosfet,usb_b,usb_adaptor,light_wire*8,aux_plug*3,aux_cable];
    ...
    constilation = task("Constilation Viewer",{paint,1});
        ...
        constillation_frame = task("Bubble Frame", {constilation,0},1);
    ...
    captian_chair = task("Captian's Chair",{building,0},1);
        ...
        captian_chair_augment = task("Augment",{captian_chair_secure,1},5);
            captian_chair_augment.materials = [two_by_two,pretty_plywood];
    ...
    computer = task("Captian's Computer",{building,0});
        ...
        computer_stand = task("Stand",{computer,0},2);
            computer_stand.materials = [monitor,Computer_stand];
    ...
    fans = task("Fans",{floor,1});
        ...
        fans_outlet_electrical = task("Outlet Electrical",{powersupply,1},3);
            fans_outlet_electrical.materials = [wires,arduino_uno,usb_b,usb_adaptor,giant_outlet,metal_box];
    ...
    crackel = task("Crackle Tube",{building,0});
        ...
        crackel_frame = task("Frame",{powersupply,1},4);
            crackel_frame.materials = [pretty_plywood,two_by_two,crackel_tube];
    ...
    infinity_mirrors = task("Infinity Mirrors",{building,0});
        ...
        infinity_mirrors_electrical = task("Electrical",{infinity_mirrors_frame,1},2);
            infinity_mirrors_electrical.materials = LED_strip;
    ...
    NASA_printer = task("NASA Printer",{top,1});
        ...
        NASA_electrical = task("Electrical",{NASA_frame,1},1);% Add wire
    ...
    main_power_controller = task("Main Power Controller",{lighting,1});
        ...
        power_electrical = task("Electrical",{main_power_controller,0},10);
            power_electrical.materials = [arduino_mega,usb_b,usb_adaptor,wires,power_switch_tail];
    ...
    conclusion = task("Conclusion",{building,0});
        ...
        captian_station = task("Captian Station",{captian_chair_augment,1},3);
        engineer_station = task("Engineer",{powersupply,1},3);
        top.starts_when(navigator_station,1)
        top.starts_when(comms_station,1)
        top.starts_when(engineer_station,1)
        conclusion_code = task("Code",{navigator_station,1},10);
            conclusion_code.starts_when(comms_station,1);
            conclusion_code.starts_when(captian_station,1);
            conclusion_code.starts_when(engineer_station,1);
        conclusion.add_subtask([navigator_station,comms_station,captian_station,engineer_station,conclusion_code]);
    building.add_subtask([floor,circuit_scramble,switch_board,lasers,pole_cover,escape_pod,constilation,blackboard,captian_chair,computer,hologram,fans,crackel,infinity_mirrors,starchart,window,ceiling,lighting,speakers,microphone,cameras,gm_computer,decorate_power_supply,create_manual,NASA_printer,main_power_controller,conclusion]);
